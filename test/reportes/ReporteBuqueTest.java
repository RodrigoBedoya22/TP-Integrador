...
class ReporteBuqueTest {
	...
	void setUp() throws Exception {
		...
		when(terminal.getNombre()).thenReturn("Terminal");
		...
		when(orden1.getNombreTerminalDestino()).thenReturn("Terminal B");
		when(orden1.getNombreTerminalOrigen()).thenReturn("Terminal");
		orden2 = mock(OrdenImportacion.class);
		when(orden2.getContenedor()).thenReturn(contenedor);
		when(orden2.getNombreTerminalDestino()).thenReturn("Terminal");
		when(orden2.getNombreTerminalOrigen()).thenReturn("Terminal B");
